module Stats
	module AllPlayerLevelGenerator
		def gen_all_player_level_report
			...
			each_zone_sdk_platform do |zone_id, sdk, platform|
				...
				records.each do |rc|
					...
					data[zone_id][sdk][platform][rc.level] ||= 0
					data[zone_id][sdk][platform][rc.level] += 1 
				end
					...
					zdata.each do |sdk, sdata|
						sdata.each do |platform, pdata|
							pdata.each do |level, num|
								...
								record = StatsModels::AllPlayerLevelReport.where(cond).first_or_initialize
								record.num = num
								record.save
							end
						end
			...
			puts "[ReportGenerator.gen_all_player_level_report]".color(:green)+" complete"
		end
	end
end