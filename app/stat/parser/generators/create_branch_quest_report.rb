module Stats
	module CreateBranchQuestGenerator
		def gen_create_branch_quest_report_by_records(records, date)
					...
					zdata.each do |sdk, sdata|
						sdata.each do |platform, pdata|
							pdata.each do |tid, tdata|
								...
								record = StatsModels::BranchQuestCreateReport.where(cond).first_or_initialize
			...
			puts "[ReportGenerator.gen_create_branch_quest_report]".color(:green)+" complete"
		end

		def gen_create_branch_quest_report
			date = @options[:date].to_date
			records = StatsModels::BranchQuestCreate.all.to_a
			
			gen_create_branch_quest_report_by_records(records, date)
		end
	end
end