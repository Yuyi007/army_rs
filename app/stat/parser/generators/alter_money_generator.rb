module Stats
	module AlterMoneyGenerator
		def gen_alter_money_report
			gen_money_consume_report
			gen_money_gain_report 
		end

		def gen_money_consume_report
			date = @options[:date].to_date
			records = StatsModels::AlterMoney.where("money < 0")
			gen_money_consume_report_by_records(records, date)
		end

		def gen_money_consume_report_by_records(records, date)
			each_zone_sdk_platform do |zone_id, sdk, platform|
				gen_money_consume_sys_report(date, zone_id, sdk, platform, records)
				gen_money_consume_level_report(date, zone_id, sdk, platform, records)
			end
			puts "[ReportGenerator.gen_money_consume_report]".color(:green)+" complete"
		end

		def gen_money_consume_level_report(date, zone_id, sdk, platform, records)
			data = {}
			players = {}
			records.each do |rc|
				data[zone_id] ||= {}
		...
		def gen_money_consume_sys_report(date, zone_id, sdk, platform, records)
			data = {}
			players = {}
			records.each do |rc|
				data[zone_id] ||= {}
				...
				reason = rc.reason
				data[zone_id][sdk][platform][reason] ||= {:money => 0, :players => 0}
				players[zone_id][sdk][platform][reason] ||= {}
			...
			data.each do |zone_id, zdata|
				zdata.each do |sdk, sdata|
					sdata.each do |platform, pdata|
						pdata.each do |reason, rdata|
							record = StatsModels::AlterMoneySys.where(:date => date, :zone_id => zone_id, 
																	 												:sdk => sdk, :platform => platform, :reason => reason).first_or_initialize
							record.money = -rdata[:money]
		...
		def gen_money_gain_report_by_records(records, date)
			each_zone_sdk_platform do |zone_id, sdk, platform|
				data = {}
				players = {}
				records.each do |rc|
				...
				data.each do |zone_id, zdata|
					zdata.each do |sdk, sdata|
						sdata.each do |platform, pdata|
							pdata.each do |reason, rdata|
								record = StatsModels::GainMoneySys.where(:date => date, :zone_id => zone_id, 
																	 												:sdk => sdk, :platform => platform, :reason => reason).first_or_initialize
				        record.money = rdata[:money]
				        record.players = rdata[:players]
				        record.save
			...
			puts "[ReportGenerator.gen_money_gain_report]".color(:green)+" complete"
		end

		def gen_money_gain_report
			date = @options[:date].to_date
			records = StatsModels::AlterMoney.where("money > 0")
			gen_money_gain_report_by_records(records, date)
		end
	end
end