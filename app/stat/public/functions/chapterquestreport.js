...
function onLaunch()
{
  var save_store = Ext.create('Ext.data.Store', {
              fields: ['date', 'tid', 'create_count', 'finish_count'],
              });
  ...
  var columns = [ { text: '日期',  dataIndex: 'date'},
                  { text: '任务',  dataIndex: 'tid'},
                  { text: '接任务人数',  dataIndex: 'create_count'},
                  { text: '完成任务人数',  dataIndex: 'finish_count'}];
 ...
 var store_zone = Ext.create('Ext.data.Store', { fields: ['zone_name', 'zone_id']});
  ...
  function onBtnQuery()
  {
    ...
    // alert(g_search_name);
    ajaxCall({
            'url'   : '/statshelper/get_chapter_request_report',
  ...
  function updateGrid(res)
  {
    ...
    for(var i=0; i<res.length; i++)
    {
      ...
      save_store.add( { date: rc['date'],
                          tid: rc['tid'],
                          create_count: rc['create_count'],
                          finish_count: rc['finish_count']});
      if (i<50)
      {
