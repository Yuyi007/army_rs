...
{ 
  var store_grid = Ext.create('Ext.data.Store', {
                fields: ['date', 'campaign', 'count'],
                });

  var columns = [ { text: '日期',  dataIndex: 'date'},
                  { text: '战斗ID',  dataIndex: 'campaign'},
                  { text: '人数',  dataIndex: 'count'}];
  ...
  { 
    ...
    g_search_name = zone_input.getRawValue() + "_" + sdk_input.getRawValue() + "_" + platform_input.getRawValue() + "_" + date_start.getRawValue() + "_" + date_end.getRawValue()+"主线副本通过人数";
    // alert(g_search_name);
    ajaxCall({ 
            'url'   : '/statshelper/get_main_quest_cam',
            'params': { sdk: sdk,
                        platform: platform,
