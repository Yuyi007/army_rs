...
function onLaunch()
{
  var store_grid =  Ext.create('Ext.data.Store', {
                      fields: ['date', 'goods', 'shop', 'cost', 'count', 'consume', 'account', 'players']
                    });
  var columns = [ { text: '日期',  dataIndex: 'date'},
                  { text: '商品',  dataIndex: 'goods'},
                  { text: '商店',  dataIndex: 'shop'},
                  { text: '消耗类型',  dataIndex: 'cost'},
                  { text: '数量',  dataIndex: 'count'},
                  { text: '花费',  dataIndex: 'consume'},
  ...
  function onBtnQuery()
  {
    ...
    g_search_name = zone_input.getRawValue() + "_" + sdk_input.getRawValue() + "_" + platform_input.getRawValue() + "_" + date_input.getRawValue() + "商城消费";

    ajaxCall({
        'url'   : '/statshelper/get_shop_consume',
