...
function onLaunch()
{
	var store_grid = Ext.create('Ext.data.Store', {
                ...
                fields: ['date', 'zone_id', 'tid', 'num', 'consume'],
                });
  var columns = [ { text: '日期',  dataIndex: 'date'},
  								{ text: '区',  dataIndex: 'zone_id'},
                  { text: '礼包',  dataIndex: 'tid'},
                  { text: '数量',  dataIndex: 'num'},
                  { text: '金额',  dataIndex: 'consume'}];
  ...
  { 
    ...
    g_search_name = zone_input.getRawValue() + "_" + sdk_input.getRawValue() + "_" + platform_input.getRawValue() + "_" +date_input.getRawValue() + "_" + "vip礼包购买报表";

    ajaxCall({ 
        'url'   : '/statshelper/get_vip_purchase_report',
