class TeamMatch3V3
	...
	def match_Pair_I(groupid, count)
		...
		while @team_infos[groupid] and @team_infos[groupid].size >= count 
			...
			if pick_team_part_I(PAIR_RIGHT, 3, 1) 				# 3
				push_team_pair()	

			elsif pick_team_part_II(PAIR_RIGHT, 2, 1, 1, 1) 		# 2 1
				push_team_pair()	

			elsif 	pick_team_part_I(PAIR_RIGHT, 1, 3) 			# 1 1 1 
				push_team_pair()
