class TeamMatch4V4
	...
	def do_match() #return [  {:left=>[team_item, team_item, ......], :right=>[team_item, team_item, ......] },  ......   ]
		...
		match_Pair_II(3, 1, 1, 1)

		# Group 2
		match_Pair_II(2, 2, 1, 1)
		match_Pair_II(2, 1, 1, 2)
	...
	def match_Pair_I(groupid, count)
		...
		while @team_infos[groupid] and @team_infos[groupid].size >= count 
			...
			if pick_team_part_I(PAIR_RIGHT, 4, 1) 		# 4		
				push_team_pair()	

			elsif pick_team_part_II(PAIR_RIGHT, 3, 1, 1, 1) 			# 3  1
				push_team_pair()	

			elsif pick_team_part_II(PAIR_RIGHT, 2, 2, 1, 1) 		# 2 2
				push_team_pair()	

			elsif pick_team_part_II(PAIR_RIGHT, 2, 1, 1, 2) 		# 2 1 1
				push_team_pair()	

			elsif 	pick_team_part_I(PAIR_RIGHT, 1, 4) 			# 1 1 1 1
				push_team_pair()
