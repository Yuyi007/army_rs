...
static int ludp_makeaddr(lua_State *L){
	const char * address = luaL_checkstring(L, 1);
	uint16_t port = (uint16_t)luaL_checkinteger(L, 2);
	// printf("<udp_makeaddr11 port:%d \n", port);
	port = htons(port);
	// printf("<udp_makeaddr22 port:%d \n", port);
	uint8_t addr[1+2+4] = {0};
	addr[0] = PROTOCOL_UDP;
	memcpy(addr+1, &port, sizeof(uint16_t));

	// *((uint16_t *)(addr+1)) = port;
	if(1 != inet_pton(AF_INET, address, addr+3)){
		return luaL_error(L, "Invalid udp address");
	}

	lua_pushlstring(L, addr, 7);
/*
	uint16_t s1 = 0;
	uint16_t s2 = 0;
	uint16_t s3 = 0;
	...
	lua_pushinteger(L, s3);*/
	
	return 1;
}

static int ludp_getserviceindex(lua_State *L){
	const char * address = luaL_checkstring(L, 1);
	uint16_t port = (uint16_t)luaL_checkinteger(L, 2);
	uint16_t servicecount = (uint16_t)luaL_checkinteger(L, 3);
	// printf("<udp_makeaddr11 port:%d \n", port);
	//port = htons(port);
	// printf("<udp_makeaddr22 port:%d \n", port);
	uint8_t addr[1+2+4] = {0};
	...
	uint32_t num = port;
	num = num + *((uint32_t*)(addr+3));

	uint32_t index = num % servicecount;
	lua_pushinteger(L, index);

	// printf("lua get_serviceindex: %d, %d, %d\n", port, *((uint32_t*)(addr+3)), index);
...
static int ludp_regclientagent(lua_State *L){
	struct CLIENT_REGINFO ci;
	uint8_t sz = sizeof(struct CLIENT_REGINFO);

	ci.gateHandle = luaL_checkinteger(L, 1);
	ci.agentbegin = luaL_checkinteger(L, 2);
	ci.agentcount = luaL_checkinteger(L, 3);

	printf("reg agents: %d, %d, %d ", ci.gateHandle, ci.agentbegin, ci.agentcount);

	uint8_t buffer[256];
	buffer[0] = 'R';
	buffer[1] = sz;
	memcpy(buffer+2, &ci, sz);

	skynet_sendsocketrequest_ext((const void*)buffer, sz+2);
	
	return 0;
}

struct SINGLE_REGINFO
{
	uint32_t gateHandle;
	uint32_t port;
	uint32_t ip;
...
static int ludp_regsingleagent(lua_State *L){
	struct SINGLE_REGINFO si;
	uint8_t sz = sizeof(struct SINGLE_REGINFO);

	si.gateHandle = luaL_checkinteger(L, 1);
	const char * address = luaL_checkstring(L, 2);
	si.port = (uint32_t)luaL_checkinteger(L, 3);
	si.agent = (uint32_t)luaL_checkinteger(L, 4);

	uint32_t addr;

	if(1 != inet_pton(AF_INET, address, &addr)){
		return luaL_error(L, "Invalid udp address");
	}
	si.ip = addr;

	printf("reg agent: %d, %d, %d, %d ", si.gateHandle, si.ip, si.port, si.agent);

	uint8_t buffer[256];
	buffer[0] = 'R';
	buffer[1] = sz;
	memcpy(buffer+2, &si, sz);
...
static int ludp_removesingleagent(lua_State *L){
	const char * address = luaL_checkstring(L, 1);
	uint16_t port = (uint16_t)luaL_checkinteger(L, 2);
	...
	if(1 != inet_pton(AF_INET, address, buffer+4)){
		return luaL_error(L, "Invalid udp address");
	}

	printf("remove agent: %d", port);

	skynet_sendsocketrequest_ext((const void*)buffer, 6+2);
...
luaopen_skynet_socketdriver(lua_State *L) {
	...
	luaL_Reg l2[] = {
		...
		{ "udp_makeaddr", ludp_makeaddr },
		{ "udp_getserviceindex", ludp_getserviceindex},
		{ "udp_regclientagent", ludp_regclientagent},
		{ "udp_regsingleagent", ludp_regsingleagent},
		{ "udp_removesingleagent", ludp_removesingleagent},
		{ NULL, NULL },
	};
