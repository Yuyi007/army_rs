...
lsyslog(lua_State *L){
	...
	int n = lua_gettop(L);
	if(n < 2){
		luaL_error(L, "invalid need arguements for function syslog(level, ...)");
		return 0;
	}

	char level = (char)luaL_checkinteger(L, 1);
	if (n <= 2) {
		lua_settop(L, 1);
		const char * s = luaL_tolstring(L, 2, NULL);
		skynet_syslog(context, level, "%s", s);
		return 0;
	}
	...
	skynet_syslog(context, level, "%s", lua_tostring(L, -1));
	return 0;
}
...
ltrashudp(lua_State *L) {
	...
	default:
		luaL_error(L, "skynet.trashudp invalid param %s", lua_typename(L,t));
	}
...
static int
lunpackext(lua_State *L) {
	uint8_t *buffer = lua_touserdata(L,1);
	//printf("unpack %d", message);
	int size = luaL_checkinteger(L,2);

	lua_pushlstring(L, buffer+size-7, 7); //client addr
	lua_pushlstring(L, buffer, size-7); //data

	if (buffer[2] == 1){
		lua_pushinteger(L, 1);

		int len = size-8;
		uint32_t sessionid = *((uint32_t*)(buffer+len-7));
		lua_pushinteger(L, sessionid);
		uint32_t gatesockid = *((uint32_t*)(buffer+len-7+4));
		lua_pushinteger(L, gatesockid);
