...
static uint32_t g_UDPGate = 0;

//share agent mode
static uint32_t g_AgentBegin = 0;
static uint32_t g_AgentCount = 0;

//single agent mode
#define AGENT_HASH_COUNT 1000
struct AGENT_HASH_ITEM
{
	uint32_t ip;
	uint16_t port;
	uint32_t agent;

	struct AGENT_HASH_ITEM* pNext;
};
struct AGENT_HASH_ITEM* g_agentHashTable[AGENT_HASH_COUNT];

static void 
init_agentHashTable(){
	memset(g_agentHashTable, 0, sizeof(struct AGENT_HASH_ITEM*)*AGENT_HASH_COUNT);
}

static void 
reg_udp_clientagent(const void *buffer, uint8_t sz){
	if (sz == sizeof(struct CLIENT_AGENTINFO)){
		struct CLIENT_AGENTINFO* ca = (struct CLIENT_AGENTINFO*)buffer;

		g_UDPGate = ca->gateHandle;
		g_AgentBegin = ca->agentbegin;
		g_AgentCount = ca->agentcount;

		return;
	}

	if (sz == sizeof(struct SINGLE_AGENTINFO)){
		//printf("reg single agent\n");
		struct SINGLE_AGENTINFO* sa = (struct SINGLE_AGENTINFO*)buffer;

		if (g_UDPGate == 0){
			g_UDPGate = sa->gateHandle;
		}

		uint32_t num = sa->port;
		num = num + sa->ip;
		uint32_t index = num % AGENT_HASH_COUNT;

		struct AGENT_HASH_ITEM* pCur = g_agentHashTable[index];

		struct AGENT_HASH_ITEM* pOK = NULL;
		while(pCur){
			if (pCur->port == sa->port  && pCur->ip == sa->ip){
				pOK = pCur;
				break;
			}
		...
		if (pOK){
			...
			struct AGENT_HASH_ITEM* pNew = malloc(sizeof(struct AGENT_HASH_ITEM));
			pNew->ip = sa->ip;
			pNew->port = sa->port;
			pNew->agent = sa->agent;

			pNew->pNext = g_agentHashTable[index];
			g_agentHashTable[index] = pNew;

			//printf("add single agent %d %d : %d\n", sa->ip, sa->port, sa->agent);
		}
	...
	if (sz == 6){
		//printf("remove agent item\n");
		uint16_t lport = *((uint16_t *)buffer);
		uint32_t lip = *((uint32_t *)(buffer+2));
		...
		struct AGENT_HASH_ITEM* pLast = NULL;
		struct AGENT_HASH_ITEM* pRemove = NULL;
		while(pCur){
			if (pCur->port == lport  && pCur->ip == lip){
				pRemove = pCur;
				break;
			}
		...
		if (pRemove){
			if (pLast == NULL){
				g_agentHashTable[index] = pRemove->pNext;
			}else{
				pLast->pNext = pRemove->pNext;
			}

			free(pRemove);
			printf("free agent hast item: %d, %d\n", lip, lport);
		}
...
uint32_t get_clientagent(uint32_t hGate, const uint8_t *buffer, uint8_t sz){
	if (sz != 7 || hGate != g_UDPGate){
		return 0;
	}

	uint16_t lport = *((uint16_t*)(buffer+1));
	lport = ntohs(lport);
	uint32_t lip = *((uint32_t*)(buffer+3));

	uint32_t num = lport;
	num = num + lip;
	uint32_t index = 0;

	if (g_AgentCount > 0){  //shareagent
		index = num % g_AgentCount;
		//printf("socket_server get_clientagent: %d, %d, %d\n", lport, lip, g_AgentBegin + index);

		return g_AgentBegin + index;
	}else {	//single agent ==> hash table
		index = num % AGENT_HASH_COUNT;
...
ctrl_cmd(struct socket_server *ss, struct socket_message *result) {
	...
	case 'U':
		...
		return -1;
	case 'R':
		reg_udp_clientagent(buffer, len);
		return -1;
	default:
...
void socket_server_sendrequest_ext(struct socket_server *ss, const void *buffer, uint8_t sz){
		if (ss == NULL) {
		return;
	}

	int fd = ss->sendctrl_fd;
