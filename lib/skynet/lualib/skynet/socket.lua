...
--[[	print("&&& socket_message UDP",id, size, data, address, socket.udpCustomCallback)
	if socket.udpCustomCallback then
		local str1 = skynet.tostring(data, size)
		skynet_core.trash(data, size)

		socket.udpCustomCallback(str1, size, address)
		return
	end--]]
	---------------
...
socket.udp_makeaddr = assert(driver.udp_makeaddr)
socket.udp_getserviceindex = assert(driver.udp_getserviceindex)
socket.udp_regclientagent = assert(driver.udp_regclientagent)
socket.udp_regsingleagent = assert(driver.udp_regsingleagent)
socket.udp_removesingleagent = assert(driver.udp_removesingleagent)
