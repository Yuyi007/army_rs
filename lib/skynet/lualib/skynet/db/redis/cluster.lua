...
function rediscluster:cmd_error()
...
function rediscluster:get_random_connection()
	...
	skynet.error("Can't reach a single startup node.")
...
function rediscluster:call(...)
	...
	while ttl > 0 do
		...
		if not key then
			self.exec_error = true
			skynet.error("No way to dispatch this command to Redis Cluster: " .. tostring(argv[1]))
			return nil
		end
		local conn
		local slot = self:keyslot(key)
		-- print("redis cmd:", argv[1], "key:", key, " slot:", slot)
		if asking then
	...
	self.exec_error = true
	skynet.error(string.format("Too many Cluster redirections?,maybe node is disconnected (last error: %q)",err))
end
