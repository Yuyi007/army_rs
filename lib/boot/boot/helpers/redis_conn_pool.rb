...
module Boot
  ...
  # Monkey patches EM::Synchrony::ConnectionPool to a new class
  class RedisConnPool

    attr_reader :ops_count
    include Loggable
    ...
    private
      ...
      def method_missing(method, *args, &blk)
        ...
        execute(async) do |conn|
          ...
          Fiber.current[:redis_ops_count] += 1

          # if method =~ /^eval/ then
          #   puts "method=#{method} args=#{args[1][:keys]} #{ops_count}"
          # else
          #   puts "method=#{method} args=#{args[0]} #{ops_count}"
          # end
