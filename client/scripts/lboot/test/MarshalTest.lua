...
local p1 = { z=0, a=1, b='user', c=false, d=function() end, e={}, f={a=1}, h=cjson.null, y=0 }
local p2 = { r=p1, s={1, 2, 3}, t=true }
p1.g = p2
local pr1 = marshal.clone(p1, {}, {})
logd("p1=%s", peek(p1))
logd("pr1=%s", peek(pr1))

local p1enc = marshal.encode(p1, {}, {['function']=false, ['userdata']=false})
logd("skip func/user: p1enc length=%d", string.len(p1enc))
local pr2 = marshal.decode(p1enc, {})
logd("skip func/user: pr2=%s", peek(pr2))

p1enc = marshal.encode(p1, {}, {['function']=false, ['userdata']=false, ['seen']=false})
logd("skip func/user/seen: p1enc length=%d", string.len(p1enc))
pr2 = marshal.decode(p1enc, {})
logd("skip func/user/seen: pr2=%s", peek(pr2))

local p1enc2 = marshal.encode(p1, {}, {['function']=false, ['userdata']=false, ['seen']=false, ['sortkeys']=true})
logd("sortkeys: p1enc length=%d", string.len(p1enc2))
assert(p1enc ~= p1enc2)
pr2 = marshal.decode(p1enc2, {})
logd("sortkeys: pr2=%s", peek(pr2))

print "MarshalTest OK"
