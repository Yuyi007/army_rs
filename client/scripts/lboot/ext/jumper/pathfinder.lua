...
if (...) then
  ...
  -- Dependencies
	local Utils     = require 'lboot/ext/jumper/core/utils'
	local Assert    = require 'lboot/ext/jumper/core/assert'
  local Heap      = require 'lboot/ext/jumper/core/bheap'
  local Heuristic = require 'lboot/ext/jumper/core/heuristics'
  local Grid      = require 'lboot/ext/jumper/grid'
  local Path      = require 'lboot/ext/jumper/core/path'
  ...
  local Finders = {
    ['ASTAR']     = require 'lboot/ext/jumper/search/astar',
    ['ASTAR_F16'] = require 'lboot/ext/jumper/search/astar_fix16',
    ['DIJKSTRA']  = require 'lboot/ext/jumper/search/dijkstra',
    ['THETASTAR'] = require 'lboot/ext/jumper/search/thetastar',
    ['BFS']       = require 'lboot/ext/jumper/search/bfs',
    ['DFS']       = require 'lboot/ext/jumper/search/dfs',
    ['JPS']       = require 'lboot/ext/jumper/search/jps',
    ['JPS_F16']   = require 'lboot/ext/jumper/search/jps_fix16',
  }
  ...
  -- pathfinder coordinates are mirrored with Unity coordinates
  -- reversely annotate grid so that clearance looks right
  -- 2015/08/19 lt
  function Pathfinder:annotateGridRev()
    assert(self._walkable, 'Finder must implement a walkable value')
    for x=self._grid._max_x,self._grid._min_x,-1 do
      for y=self._grid._min_y,self._grid._max_y,1 do
        ...
        if self._grid:isWalkableAt(x,y,self._walkable) then
          ...
          if nr and nrd and nd then
            ...
            m = (nr._clearance[self._walkable] or 0)<m and (nr._clearance[self._walkable] or 0) or m
            -- node._clearance[self._walkable] = m+1
            -- add one rule to check up and left obstacles, if found,
            -- set clearance to 1, to avoid large units moving too close to obstacles
            -- This is not needed when we have proper collision avoidance steering
            -- in good effects
            if self._grid:isWalkableAt(node._x-1, node._y, self._walkable) and
              self._grid:isWalkableAt(node._x-1, node._y+1, self._walkable) and
              self._grid:isWalkableAt(node._x, node._y+1, self._walkable) then
              node._clearance[self._walkable] = m+1
            else
              node._clearance[self._walkable] = 1
            end
  ...
  -- @treturn {string,...} array of built-in finders names.
	...
	-- for i = 1, #finders do local finderName = finders[i]
	--   print(i, finderName)
	-- end
  ...
  -- @treturn {string,...} array of heuristic names.
	...
	-- for i = 1, #heur do local heuristicName = heur[i]
	--   ...
	-- end
