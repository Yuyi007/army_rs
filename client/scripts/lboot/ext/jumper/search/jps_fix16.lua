-- fixmath (16.16) version of Jump Point search algorithm
-- 2015-08-18 lt

if (...) then

  -- Dependancies
  local Heuristics = require 'lboot/ext/jumper/core/heuristics'
  ...
  local fm = fixmath
  local from_int = fm.fix16_from_int
  local to_int = fm.fix16_to_int
    ...
    for i = #neighbours,1,-1 do
      ...
      if jumpNode and not skip then
        ...
        if not jumpNode._closed then
          local extraG = Heuristics.EUCLIDIAN_F16(jumpNode, node)
          local newG = node._g + extraG
          if not jumpNode._opened or newG:lt(jumpNode._g) then
