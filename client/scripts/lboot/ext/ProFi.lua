--[[
  Add memory allocation stats to ProFi
  Requires lua_xt.c trace_alloc functions.
]]
...
local ProFi = {}
local onDebugHook, sortByDurationDesc, sortByCallCount, getTime, getAllocStats
...
function ProFi:start( param )
  ...
  self.startAllocCount, self.startAllocSize, self.startFreeCount, self.startFreeSize = getAllocStats()
  self:startHooks()
end
...
function ProFi:stop()
  ...
  self.stopAllocCount, self.stopAllocSize, self.stopFreeCount, self.stopFreeSize = getAllocStats()
...
function ProFi:writeProfilingReportsToFile( reports, file )
  ...
  file:write( header )
  for i = 1, # reports  do local funcReport =  reports [i]
...
function ProFi:writeMemoryReportsToFile( reports, file )
  ...
  for i = 1, # reports  do local memoryReport =  reports [i]
...
function ProFi:writeInpsectionsToFile( inspections, file )
  ...
  file:write('\n==^ INSPECT ^======================================================================================================== COUNT ===\n')
  for i = 1, # inspectionsList  do local inspection =  inspectionsList [i]
...
function ProFi:resetReports( reports )
  for i = 1, # reports  do local report =  reports [i]
    report.timer = 0
    report.count = 0
...
function ProFi:onFunctionCall( funcInfo )
  ...
  funcReport.callAllocCount, funcReport.callAllocSize, funcReport.callFreeCount, funcReport.callFreeSize = getAllocStats()
...
function ProFi:onFunctionReturn( funcInfo )
  ...
  if funcReport.callAllocCount then
    local allocCount, allocSize, freeCount, freeSize = getAllocStats()
    funcReport.allocCount = funcReport.allocCount + (allocCount - funcReport.callAllocCount)
    funcReport.allocSize = funcReport.allocSize + (allocSize - funcReport.callAllocSize)
    funcReport.freeCount = funcReport.freeCount + (freeCount - funcReport.callFreeCount)
    funcReport.freeSize = funcReport.freeSize + (freeSize - funcReport.callFreeSize)
  end

  -- if string.match(funcInfo.source, 'ValueTypeScript') and
  --   funcInfo.linedefined == 866 then
  --   self:doInspection( {levels = 8}, funcReport )
  --   logd('ValueTypeScript inspect=%s', peek(funcReport.inspections))
  -- end
end
...
local xt = require 'xt'
getAllocStats = xt.get_alloc_stats
