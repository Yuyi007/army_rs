...
do
  ...
  local function recursive_proceed(t, added, rec_info, after, started)
    ...
    if t_type == "table" then
      ...
      if type(mt.__persist) == 'function' then
        return recursive_proceed(mt.__persist(t)(), added, rec_info, after, started)
      end
  ...
  local function recursive_proceed_simple(t, added)
    ...
    if t_type == "table" then
      local mt = getmetatable(t) or {}
      if type(mt.__persist) == 'function' then
        return recursive_proceed_simple(mt.__persist(t)(), added)
      end
