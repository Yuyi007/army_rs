...
-- local default_start_tag, default_end_tag = '#{', '}#'
local default_start_tag, default_end_tag = '<%', '%>'
...
local function include_fold(template, start_tag, end_tag, fold_func, init_func)
  ...
  start_tag = start_tag or default_start_tag
  end_tag = end_tag or default_end_tag
...
local mt52 = { __index = rawget(_G, '_ENV') }
...
function slt2.render_co(t, env)
  ...
  if setfenv == nil then -- lua 5.2
    ...
    f = assert(load(t.code, t.name, 't', env or rawget(_G, '_ENV')))
  else -- lua 5.1
    if env ~= nil then
...
--[[
  helpers to ease use of slt2
  2014-06-10 lt

  usage: template('<p>hello</p>'):render({ vars => 'vars' })
]]--
function template(str)
  local tpl = slt2.loadstring(str)
  tpl.render = function(self, vars)
    return slt2.render(self, vars)
  end
  return tpl
end
